...
class singleRegionOutputTest(tf.test.TestCase):
    ...
    def test_single_gt_A(self):
        expectation = np.array([[0, 0, 1, 1, 0], [4, 6, 9, 9, -1], [5, 5, 7, 7, 1]],
                dtype=np.float32)
        self.single_gt_template(.2, .5, expectation)

    def test_single_gt_B(self):
        expectation = np.array([[0, 0, 1, 1, 0], [4, 6, 9, 9, 0], [5, 5, 7, 7, 1]],
                dtype=np.float32)
    ...
    def test_iou_free_pos(self):
        ...
        expectation = np.array([0, 3, -1, -1, 1], dtype=np.float32)
        self.multiple_gt_template(.03, .99, expectation)

    def test_iou_B(self):
        expectation = np.array([0, 3, 1, 0, 1], dtype=np.float32)
