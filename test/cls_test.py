...
class ClsTest(unittest.TestCase):
    ...
    def test_roi_pooling(self):
        """
        Tests the Region of Interest pooling layer (pool5) using actual activation data from
        a reference network.
    ...
    def test_fc7(self):
        ...
        def runGraph(self):
            with tf.Session() as sess, tf.device("/gpu:0"):
                ...
                cls.setUp(pool5, 7, 7, 512, namespace="rcnn")
    ...
    def test_cls_score(self):
        ...
        def runGraph(self):
            with tf.Session() as sess, tf.device("/gpu:0"):
                ...
                loadNetVars.extractLayers("rcnn", s.DEF_FRCNN_WEIGHTS_PATH,
                    s.DEF_FRCNN_BIASES_PATH)
                cls.setUp(tf.placeholder("float", name="pool5"), 7, 7, 512, namespace="rcnn")
                sess.run(tf.global_variables_initializer())
