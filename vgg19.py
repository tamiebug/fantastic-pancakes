...
class Vgg19():
	...
	def buildGraph(self, img, train=False, weightsPath=s.DEF_WEIGHTS_PATH,
		...
		# If the variable cutoff is set to a string besides the empty string,
		# then the created Vgg19 network is cut off at the desired cutoff layer.
		
		# Extracts the information from .npz files and puts them into properly scoped tf.Variable(s)
		...
		def createConvLayer(bottom, name, trainable=True):
			...
			with tf.variable_scope(name) as scope:
				conv = tf.nn.conv2d(bottom, tf.get_variable("Weights"), [1,1,1,1], padding="SAME")
		...
		def createFirstFcLayer(bottom, name, trainable=True):
			...
			with tf.variable_scope(name) as scope:
				...
				layer = tf.nn.bias_add(tf.matmul(flattenedInput, tf.get_variable("Weights")), tf.get_variable("Bias"))
		...
		def createFcLayer(bottom, name, trainable=True):
			...
			with tf.variable_scope(name) as scope:
				layer = tf.nn.bias_add(tf.matmul(bottom, tf.get_variable("Weights")), tf.get_variable("Bias"))
		...
		with tf.variable_scope(self.namespace) as scope:
			...
			# We assume that no one wants to cutoff before the relu.
			if not (cutoff=='fc6' or cutoff=='relu6'):
			...
			if not(cutoff=='fc7' or cutoff=='relu7'): 
			...
			# Return last layer, even though it's easy to access from the object.
