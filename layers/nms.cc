...
static inline float ComputeIOU(typename TTypes<float, 2>::ConstTensor boxes,
  ...
  const float area_i = (ymax_i - ymin_i + 1.0) * (xmax_i - xmin_i + 1.0);
  const float area_j = (ymax_j - ymin_j + 1.0) * (xmax_j - xmin_j + 1.0);
...
REGISTER_KERNEL_BUILDER(Name("NMS").Device(DEVICE_CPU),
